...
export class TipoRubroDao {
    ...
    obtenerVigentes(): Observable<TipoRubro[]> {
        return this.http.get<TipoRubro[]>(environment.tanatosService.apiUrl + '/TipoRubro/Vigentes');
    }

    obtenerPorVigencia(vigencia: boolean | null): Observable<TipoRubro[]> {
        return this.http.get<TipoRubro[]>(environment.tanatosService.apiUrl + `/TipoRubro/PorVigencia/${vigencia}`);
    }

    crear(entrada: TipoRubro): Observable<TipoRubro> {
        return this.http.post<TipoRubro>(environment.tanatosService.apiUrl + '/TipoRubro/', entrada);
    }
